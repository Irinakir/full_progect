...
const rowsPerPage = 3,
      medalsRowsCol = document.querySelectorAll('#all_products tbody tr'),
...
function showOnlyRowsFromPage(currentPage, rowsPerPage) {
  const {from, to} = getRowsForShow(currentPage, rowsPerPage),
        hiddenElCol = document.querySelectorAll(
          `#all_products tbody tr:nth-child(-n + ${from - 1}):not(.hidden), #all_products tbody tr:nth-child(n + ${to + 1}):not(.hidden)`
        ),
        shownElCol = document.querySelectorAll(
          `#all_products tbody tr:nth-child(n + ${from}):nth-child(-n + ${to}).hidden`
        );
